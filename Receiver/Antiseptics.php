...
// Select all  previous and next work
 ...
 $sqlTotalid2 = "SELECT inventory_id FROM inventory where category='Antiseptics';";
 ...
 if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter'])  ) {
      $MedQuery = "select * from 	inventory  where category='Antiseptics' and med_name like '%$medf%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) ){
      $MedQuery = "select * from inventory where category='Antiseptics' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
 
 
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter'])){
      $MedQuery = "select * from inventory where category='Antiseptics' and med_name like '%$medf%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'date'){
     $MedQuery = "select * from inventory where category='Antiseptics' order by expdate DESC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from inventory where category='Antiseptics' order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from inventory where category='Antiseptics' LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from inventory where category='Antiseptics' LIMIT $startingRow, $perPage;";
  }
...
// $sqlshow = "select * from medicine where category='Antiseptics' order by med_id DESC LIMIT $startingRow, $perPage;";
$results=mysqli_query($conn,$MedQuery);

  $sqlTotalid = "SELECT COUNT(*) as t_c FROM inventory where category='Antiseptics';";
  ...
  $allmedname = "select med_name , COUNT(*) AS CountOf from inventory where category='Antiseptics' GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  
  $allmedbrand = "SELECT brand , COUNT(*) AS CountOf FROM inventory where category='Antiseptics' GROUP BY brand ASC HAVING COUNT(*)>0";
      ...
      if($updatequantity >= 0)
         {
            ...
            $sql = "INSERT INTO receiverrequest (	req_date,med_name,	expdate,	batchid,brand,category,quantity,location,rec_ID,	inventory_id) VALUES ('".$currentD."','".$medname."','".$expdate."', '".$batchid."', '".$brand1."', '".$cat."','".$quantity."','".$loc."','".$recid."','".$med_id."')";
                  ...
                  if($t)
                  {
                    // header('location: Antiseptics.php');
...
<form action="Antiseptics.php" method="post" onsubmit="return checkforblank1()" name="frm">
  ...
  <div class="form-group">
      ...
      <select id="inputState" class="form-control" name="t_ype" id="cate">
        <option value="Antiseptics" selected >Antiseptics</option>
      </select>
    </div>
...
<form action="Antiseptics.php" method="post" name="frm">
...
<!-- //********* */ -->
     ...
     <div class="refresh" id ="three"> <a href="Antiseptics.php"> <input type="submit" value="Refesh"></a> </div>
              ...
              <table>
                    ...
                    <?php while($row = $results->fetch_assoc()):  ?>
                           ...
                           ?>
                            ...
                            <td>  
                          <a href="Antiseptics.php?edit= <?php echo $row['inventory_id']; ?>" class="edit_btn">Request</a> 
