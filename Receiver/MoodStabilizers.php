...
// Select all  previous and next work
 ...
 $sqlTotalid2 = "SELECT inventory_id FROM inventory where category='MoodStabilizers';";
 ...
 if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter'])  ) {
      $MedQuery = "select * from 	inventory  where category='MoodStabilizers' and med_name like '%$medf%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) ){
      $MedQuery = "select * from inventory where category='MoodStabilizers' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
 
 
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter'])){
      $MedQuery = "select * from inventory where category='MoodStabilizers' and med_name like '%$medf%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'date'){
     $MedQuery = "select * from inventory where category='MoodStabilizers' order by expdate DESC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from inventory where category='MoodStabilizers' order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from inventory where category='MoodStabilizers' LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from inventory where category='MoodStabilizers' LIMIT $startingRow, $perPage;";
  }
...
// $sqlshow = "select * from medicine where category='MoodStabilizers' order by med_id DESC LIMIT $startingRow, $perPage;";
$results=mysqli_query($conn,$MedQuery);

  $sqlTotalid = "SELECT COUNT(*) as t_c FROM inventory where category='MoodStabilizers';";
  ...
  $allmedname = "select med_name , COUNT(*) AS CountOf from inventory where category='MoodStabilizers' GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  
  $allmedbrand = "SELECT brand , COUNT(*) AS CountOf FROM inventory where category='MoodStabilizers' GROUP BY brand ASC HAVING COUNT(*)>0";
      ...
      if($updatequantity >= 0)
         {
            ...
            $sql = "INSERT INTO receiverrequest (	req_date,med_name,	expdate,	batchid,brand,category,quantity,location,rec_ID,	inventory_id) VALUES ('".$currentD."','".$medname."','".$expdate."', '".$batchid."', '".$brand1."', '".$cat."','".$quantity."','".$loc."','".$recid."','".$med_id."')";
                  ...
                  if($t)
                  {
                    // header('location: MoodStabilizers.php');
...
<form action="MoodStabilizers.php" method="post" onsubmit="return checkforblank1()" name="frm">
  ...
  <div class="form-group">
      ...
      <select id="inputState" class="form-control" name="t_ype" id="cate">
        <option value="MoodStabilizers" selected >MoodStabilizers</option>
      </select>
    </div>
...
<form action="MoodStabilizers.php" method="post" name="frm">
...
<!-- //********* */ -->
     ...
     <div class="refresh" id ="three"> <a href="MoodStabilizers.php"> <input type="submit" value="Refesh"></a> </div>
...
<div class="custom_select">
 <form action="MoodStabilizers.php" method="get">
      <fieldset>
...
<div>
              ...
              <table>
                    ...
                    <?php while($row = $results->fetch_assoc()):  ?>
                           ...
                           ?>
                            ...
                            <td>  
                          <a href="MoodStabilizers.php?edit= <?php echo $row['inventory_id']; ?>" class="edit_btn">Request</a> 
 ...
 <div class="btn_next_pre">
          ...
          <a class="btnpg" href="MoodStabilizers.php?x=<?php echo $a-1; ?>" >Previous</a>
          <?php endif; ?>

    <?php
      for($i=1; $i<=$np; $i++){ ?>
        <a class="btnpg" href="MoodStabilizers.php?x=<?php echo $i; ?>"><?php echo $i; ?></a>
    <?php
      }
    ?>
  <?php if (($perPage+$startingRow) <= ($totalc-1)): ?>
    <a class="btnpg" href="MoodStabilizers.php?x=<?php echo $a+1; ?>" >Next</a>
    <?php endif; ?>  
