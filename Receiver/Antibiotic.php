...
// Select all  previous and next work
 ...
 $sqlTotalid2 = "SELECT inventory_id FROM inventory where category='Antibiotic';";
 ...
 if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter'])  ) {
      $MedQuery = "select * from 	inventory  where category='Antibiotic' and med_name like '%$medf%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) ){
      $MedQuery = "select * from inventory where category='Antibiotic' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
 
 
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter'])){
      $MedQuery = "select * from inventory where category='Antibiotic' and med_name like '%$medf%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'date'){
     $MedQuery = "select * from inventory where category='Antibiotic' order by expdate DESC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from inventory where category='Antibiotic' order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from inventory where category='Antibiotic' LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from inventory where category='Antibiotic' LIMIT $startingRow, $perPage;";
  }
...
$results=mysqli_query($conn,$MedQuery);
  ...
  $sqlTotalid = "SELECT COUNT(*) as t_c FROM inventory where category='Antibiotic';";
  ...
  $allmedname = "select med_name , COUNT(*) AS CountOf from inventory where category='Antibiotic' GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  
  $allmedbrand = "SELECT brand , COUNT(*) AS CountOf FROM inventory where category='Antibiotic' GROUP BY brand ASC HAVING COUNT(*)>0";
      ...
      if($updatequantity >= 0)
         {
            ...
            $sql = "INSERT INTO receiverrequest (	req_date,med_name,	expdate,	batchid,brand,category,quantity,location,rec_ID,	inventory_id) VALUES ('".$currentD."','".$medname."','".$expdate."', '".$batchid."', '".$brand1."', '".$cat."','".$quantity."','".$loc."','".$recid."','".$med_id."')";
                  ...
                  if($t)
                  {
                    // header('location: Antibiotic.php');
...
<form action="Antibiotic.php" method="post" onsubmit="return checkforblank1()" name="frm">
...
<form action="Antibiotic.php" method="post" name="frm">
...
<!-- //********* */ -->
     ...
     <div class="refresh" id ="three"> <a href="Antibiotic.php"> <input type="submit" value="Refesh"></a> </div>
...
<div class="custom_select">
 <form action="Antibiotic.php" method="get">
      <fieldset>
...
<div>
              ...
              <table>
                    ...
                    <?php while($row = $results->fetch_assoc()):  ?>
                           ...
                           ?>
                            ...
                            <td>  
                          <a href="Antibiotic.php?edit= <?php echo $row['inventory_id']; ?>" class="edit_btn">Request</a> 
 ...
 <div class="btn_next_pre">
          ...
          <a class="btnpg" href="Antibiotic.php?x=<?php echo $a-1; ?>" >Previous</a>
          <?php endif; ?>

    <?php
      for($i=1; $i<=$np; $i++){ ?>
        <a class="btnpg" href="Antibiotic.php?x=<?php echo $i; ?>"><?php echo $i; ?></a>
    <?php
      }
    ?>
  <?php if (($perPage+$startingRow) <= ($totalc-1)): ?>
    <a class="btnpg" href="Antibiotic.php?x=<?php echo $a+1; ?>" >Next</a>
    <?php endif; ?>  
