...
$mes="";
 ...
 $sqlTotalid2 = "SELECT med_id FROM medicine  where category='MoodStabilizers';";
 ...
 if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter'])  ) {
      $MedQuery = "select * from medicine  where category='MoodStabilizers' and med_name like '%$medf%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) ){
      $MedQuery = "select * from medicine where category='MoodStabilizers' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
 
 
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter'])){
      $MedQuery = "select * from medicine where category='MoodStabilizers' and med_name like '%$medf%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'date'){
     $MedQuery = "select * from medicine where category='MoodStabilizers' order by expdate DESC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from medicine where category='MoodStabilizers' order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from medicine where category='MoodStabilizers' LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from medicine where category='MoodStabilizers' LIMIT $startingRow, $perPage;";
  }
...
$results=mysqli_query($conn,$MedQuery);
  ...
  $sqlTotalid = "SELECT COUNT(*) as t_c FROM medicine where category='MoodStabilizers';";
  ...
  $allmedname = "select med_name , COUNT(*) AS CountOf from medicine where category='MoodStabilizers' GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  
  $allmedbrand = "SELECT brand , COUNT(*) AS CountOf FROM medicine where category='MoodStabilizers' GROUP BY brand ASC HAVING COUNT(*)>0";
...
<!-- <div class="together"> -->
<form action="MoodStabilizers.php" method="post" onsubmit="return checkforblank()" name="frm">
...
<!--  -->
<!-- <div class="together"> -->
<div class="refresh" id ="two"> <a href="MoodStabilizers.php"> <input type="submit" value="Refesh"></a> </div>
 ...
 <div class="displayData">
              <table>
                    ...
                    <?php while($row = $results->fetch_assoc()):  ?>
                            ...
                            <td>  
                          <a href="MoodStabilizers.php?edit= <?php echo $row['med_id']; ?>" class="edit_btn">Add</a> 
