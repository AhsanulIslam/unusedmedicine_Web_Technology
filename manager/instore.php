...
if (isset($_GET['edit']))
{
    ...
    $rec = mysqli_query($conn,"select * from instore where id = '".$med_id."'");
    ...
    $sql = "INSERT INTO inventory (med_name,expdate, batchid, brand,category,quantity) VALUES ('".$medname."','".$expdate."', '".$batchid."', '".$brand1."', '".$cat."', '" .$quan."')";
       mysqli_query($conn, $sql);

       $s = "delete from instore where id = '$med_id'";
       mysqli_query($conn, $s);
       header('location: instore.php');
}
...
// Select all  previous and next work
 ...
 $sqlTotalid2 = "SELECT id FROM instore";
 ...
 if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter'])  ) {
      $MedQuery = "select * from instore where med_name like '%$medf%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) ){
      $MedQuery = "select * from instore where location like '%$med_brand%' limit $startingRow, $perPage";
  }
 
 
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter'])){
      $MedQuery = "select * from instore where med_name like '%$medf%' and location like '%$med_brand%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'location'){
     $MedQuery = "select * from instore order by location ASC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from instore order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from instore LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from instore LIMIT $startingRow, $perPage;";
  }
...
$results=mysqli_query($conn,$MedQuery);
  ...
  $sqlTotalid = "SELECT COUNT(*) as t_c FROM instore ";
  ...
  $allmedname = "select med_name , COUNT(*) AS CountOf from instore GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  
  $allmedbrand = "SELECT location , COUNT(*) AS CountOf FROM instore GROUP BY location ASC HAVING COUNT(*)>0";
...
<form action="instore.php" method="post" onsubmit="return checkforblank()" name="frm">
...
<div class="custom_select">
 <form action="instore.php" method="get">
      <fieldset>
...
<div>
 ...
 <div class="displayData">
              <table>
                    ...
                    <?php while($row = $results->fetch_assoc()):  ?>
                            ...
                            <td>  
                          <a href="instore.php?edit= <?php echo $row['id']; ?>" class="edit_btn">Add To Inventory</a> 
 ...
 <footer class="btn_next_pre">
          ...
          <a class="btnpg" href="instore.php?x=<?php echo $a-1; ?>" >Previous</a>
          <?php endif; ?>

    <?php
      for($i=1; $i<=$np; $i++){ ?>
        <a class="btnpg" href="instore.php?x=<?php echo $i; ?>"><?php echo $i; ?></a>
    <?php
      }
    ?>
  <?php if (($perPage+$startingRow) <= ($totalc-1)): ?>
    <a class="btnpg" href="instore.php?x=<?php echo $a+1; ?>" >Next</a>
    <?php endif; ?>  
