...
if (isset($_GET['edit']))
{
    ...
    $rec = mysqli_query($conn,"select * from receiverrequest where req_id = '".$med_id."'");
    ...
    $brand1 =$record['brand'];
    $cat =$record['category'];
    $med_id = $record['req_id'];
    $loc = $record['location'];
    $quan = $record['quantity'];
    $qu =  mysqli_query($conn,"select receiverrequest.location ,deliveryboy.DBoy_ID,deliveryboy.	DBoy_FNAME, deliveryboy.DBoy_ADDRESS from receiverrequest,deliveryboy where receiverrequest.req_id = '".$med_id."' and receiverrequest.location like deliveryboy.DBoy_ADDRESS;");
        $record2 = mysqli_fetch_array($qu);
        if($record2 >0){
        $delid =$record2['DBoy_ID'];
        $delnam =$record2['DBoy_FNAME'];}
        else{
            $notavailable ="Not available Delivery Boy in this location";
        }
}
...
$batchid2  = $_POST['batchid1'];
$brand1  =$_POST['brand'];
$cat  =$_POST['t_ype'];
$dboy_id  =$_POST['delboyid'];
$quan  =$_POST['quantity'];
$loca  =$_POST['location'];

       $sql = "INSERT INTO donnersentdelivery (	type,med_name,expdate, batchid, brand,category,quantity,location,DBoy_ID) VALUES ('Receiver','".$medname."','".$expdate."', '".$batchid2."', '".$brand1."', '".$cat."', '".$quan."', '".$loca."', '".$dboy_id."')";
       mysqli_query($conn, $sql);

        $s = "delete from receiverrequest where req_id = '$med_id'";
       mysqli_query($conn, $s);

      header('location: deliveryReceiver.php');
}
...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@9"></script>
 ...
 <script>
                function checkforblank(){
                 ...
                 else if(document.getElementById('delname').value== "") {
                 Swal.fire('Sorry!','Expired Date can not be empty!','error')
                  return false;
                  }
                   else{
                    Swal.fire('Thanks..!!','Delivery Boy Assign Successfully!!!','success')
                  return true;
                  }
...
<div id="notavailable"> <?php echo $notavailable; ?> </div>


<form action="deliveryReceiver.php" method="post" onsubmit="return checkforblank()" name="frm">
              ...
              <input type="text" name="m_name" id="mnam" size="40" heigth="50px" placeholder="Medicine name" value="<?php echo $medname; ?>">
               <input type="date" name="expdat"  id="dat" size="40" placeholder="Expired Date" value="<?php echo $expdate; ?>">
                ...
                <!-- <input type="text" name="batchid1"  id="batchid1" size="40" placeholder="Batch id" value="<?php //echo $batchid; ?>"> -->
                 ...
                 <input type="text" name="location" id="brand" size="40"placeholder="location" value="<?php echo $loc; ?>">                  </div>  <!--hidden div-->
                 <input type="text" name="delboyid" id="delid" size="40"placeholder="Delivery Boy id" value="<?php echo $delid; ?>">
                 <input type="text" name="delboynam" id="delname" size="46"placeholder="Available Delivery Boy Name" value="<?php echo $delnam; ?>">
                 <div hidden>  <input type="text" name="quantity" id="brand" size="40"placeholder="quantity" value="<?php echo $quan; ?>">  </div>
                 
            </div>

            <div class="addbut" id="one">  <input type="submit" value="Assign" name="assign"></div>
</form>


<div class="refresh" id ="two"> <a href="deliveryDoneReceiver.php"> <input type="submit" value="Status"></a> </div>
...
<div class="custom_select">
 <form action="deliveryReceiver.php" method="get">
      <fieldset>
...
<div>
 ...
 <div class="displayData">
              <table>
                    ...
                    <?php while($row = $results->fetch_assoc()):  ?>
                            ...
                            <td>  
                          <a href="deliveryReceiver.php?edit= <?php echo $row['req_id']; ?>" class="edit_btn">Assign</a> 
 ...
 <footer class="btn_next_pre">
          ...
          <a class="btnpg" href="deliveryReceiver.php?x=<?php echo $a-1; ?>" >Previous</a>
          <?php endif; ?>

    <?php
      for($i=1; $i<=$np; $i++){ ?>
        <a class="btnpg" href="deliveryReceiver.php?x=<?php echo $i; ?>"><?php echo $i; ?></a>
    <?php
      }
    ?>
  <?php if (($perPage+$startingRow) <= ($totalc-1)): ?>
    <a class="btnpg" href="deliveryReceiver.php?x=<?php echo $a+1; ?>" >Next</a>
    <?php endif; ?>  
