...
// id count for total number of medicine show 
  $sqlTotalid2 = "SELECT notexitid FROM notexit;";
  ...
  if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])) {
      $MedQuery = "select * from notexit  where med_name like '%$medf%' limit $startingRow, $perPage";
  }
      ...
      $MedQuery = "select * from notexit where brand like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where category like '%$med_cate%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where req_date like '%$med_donate_date%' limit $startingRow, $perPage";
  }
  // combination 2
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and category like '%$med_cate%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and req_date like '%$med_donate_date%' DESC limit $startingRow, $perPage";
  }


  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where category like '%$med_cate%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where req_date like '%$med_donate_date%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
 else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where req_date like '%$med_donate_date%' and category like '%$med_cate%' limit $startingRow, $perPage";
  }

  // combination 3
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and brand like '%$med_brand%' and category like '%$ %' limit $startingRow, $perPage";
  }
   else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and  req_date like '%$med_donate_date%' and category like '%$med_cate%' limit $startingRow, $perPage";
  }
    else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and  req_date like '%$med_donate_date%' and  brand like '%$med_brand%' limit $startingRow, $perPage";
  }
   else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and  req_date like '%$med_donate_date%' and  brand like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where  category like '%$med_cate%' and req_date like '%$med_donate_date%' and brand like '%$med_brand%' limit $startingRow, $perPage";
  }
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from notexit where med_name like '%$medf%' and brand like '%$med_brand%' and category like '%$med_cate%'and req_date like '%$med_donate_date%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'date'){
     $MedQuery = "select * from notexit order by req_date DESC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from notexit order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from notexit LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from notexit limit $startingRow, $perPage";
  }
...
// $sqlshow = "select * from notexit order by med_id DESC LIMIT $startingRow, $perPage ;";
$results=mysqli_query($conn,$MedQuery);



  $sqlTotalid = "SELECT COUNT(*) as t_c FROM notexit";
...
// same name show in select option one time
  $allmedname = "select med_name , COUNT(*) AS CountOf from notexit GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  $allmedbrand = "SELECT brand , COUNT(*) AS CountOf FROM notexit GROUP BY brand ASC HAVING COUNT(*)>0";
  $resultAllBrand = mysqli_query($conn, $allmedbrand);
  
  
  $allmedcate = "SELECT category , COUNT(*) AS CountOf FROM notexit GROUP BY category ASC HAVING COUNT(*)>0";
  $resultAllcate = mysqli_query($conn, $allmedcate);


  $allmeddontdate = "SELECT req_date , COUNT(*) AS CountOf FROM notexit GROUP BY req_date ASC HAVING COUNT(*)>0";
...
<div class="custom_select">
 <form action="newmed.php" method="get">
      <fieldset>
...
<div class="btn_next_pre">
          ...
          <a class="btnpg" href="newmed.php?x=<?php echo ($a-1); ?>" >Previous</a>
          <?php endif; ?>

          <?php
            for($i=1; $i<=$np; $i++){ ?>
              <a class="btnpg" href="newmed.php?x=<?php echo $i; ?>"><?php echo $i; ?></a>
          <?php
            }
          ?>
        <?php if (($perPage+$startingRow) <= ($totalc-1)): ?>
          <a class="btnpg" href="newmed.php?x=<?php echo ($a+1); ?>" >Next</a>
          <?php endif; ?>  
