...
// id count for total number of medicine show 
  $sqlTotalid2 = "SELECT req_id FROM receiverrequest;";
  ...
  if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])) {
      $MedQuery = "select * from receiverrequest  where med_name like '%$medf%' limit $startingRow, $perPage";
  }
      ...
      $MedQuery = "select * from receiverrequest where location like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where category like '%$med_cate%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where req_date like '%$med_donate_date%' limit $startingRow, $perPage";
  }
  // combination 2
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and location like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and category like '%$med_cate%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and req_date like '%$med_donate_date%' DESC limit $startingRow, $perPage";
  }


  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where category like '%$med_cate%' and location like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where req_date like '%$med_donate_date%' and location like '%$med_brand%' limit $startingRow, $perPage";
  }
 else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where req_date like '%$med_donate_date%' and category like '%$med_cate%' limit $startingRow, $perPage";
  }

  // combination 3
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and location like '%$med_brand%' and category like '%$ %' limit $startingRow, $perPage";
  }
   else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and  req_date like '%$med_donate_date%' and category like '%$med_cate%' limit $startingRow, $perPage";
  }
    else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and  req_date like '%$med_donate_date%' and  location like '%$med_brand%' limit $startingRow, $perPage";
  }
   else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and  req_date like '%$med_donate_date%' and  location like '%$med_brand%' limit $startingRow, $perPage";
  }
  else if(isset($_GET['filter']) &&  empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where  category like '%$med_cate%' and req_date like '%$med_donate_date%' and location like '%$med_brand%' limit $startingRow, $perPage";
  }
// combination all
  else if(isset($_GET['filter']) &&  !empty($_GET['MeddicineFilter']) && !empty($_GET['brandFilter']) && !empty($_GET['categoryFilter']) && !empty($_GET['dateFilter'])){
      $MedQuery = "select * from receiverrequest where med_name like '%$medf%' and location like '%$med_brand%' and category like '%$med_cate%'and req_date like '%$med_donate_date%' limit $startingRow, $perPage";
  }

  // sort
else if(isset($_GET['sort1'])  && $_GET['sort'] == 'date'){
     $MedQuery = "select * from receiverrequest order by req_date DESC limit $startingRow, $perPage";
  }
   else if(isset($_GET['sort1'])  && $_GET['sort'] == 'name'){
     $MedQuery = "select * from receiverrequest order by med_name ASC limit $startingRow, $perPage";
  }
  else if(isset($_GET['showall'])){ $perPage = $idCount;
    $MedQuery = "select * from receiverrequest LIMIT $startingRow, $perPage";
  }
  else{
      $MedQuery = "select * from receiverrequest limit $startingRow, $perPage";
  }
...
$results=mysqli_query($conn,$MedQuery);
  ...
  $sqlTotalid = "SELECT COUNT(*) as t_c FROM receiverrequest";
...
// same name show in select option one time
  $allmedname = "select med_name , COUNT(*) AS CountOf from receiverrequest GROUP BY med_name HAVING COUNT(*)>0";
  $resultAllmedname = mysqli_query($conn, $allmedname);

  $allmedbrand = "SELECT location , COUNT(*) AS CountOf FROM receiverrequest GROUP BY location ASC HAVING COUNT(*)>0";
  $resultAllBrand = mysqli_query($conn, $allmedbrand);
  
  
  $allmedcate = "SELECT category , COUNT(*) AS CountOf FROM receiverrequest GROUP BY category ASC HAVING COUNT(*)>0";
  $resultAllcate = mysqli_query($conn, $allmedcate);


  $allmeddontdate = "SELECT req_date , COUNT(*) AS CountOf FROM receiverrequest GROUP BY req_date ASC HAVING COUNT(*)>0";
...
.newmed input[type="submit"]:hover {
  cursor: pointer;
  background: #616161;
  color: rgb(255, 255, 255);
}
...
<div class="newmed" id ="one"> <a href="newmed.php"> <input type="submit" value="New Medicine Request"></a> </div>
<div class="refresh" id ="two"> <a href="receiverList.php"> <input type="submit" value="Refresh"></a> </div>


<div class="total">
    <h2>Total Request : <?php echo $idCount; ?></h2>
</div>


<div class="custom_select">
 <form action="receiverList.php" method="get">
      <fieldset>
       ...
       <select name="categoryFilter">
        ...
        <select name="dateFilter">
          <option value="" disabled selected>Request Date...</option>
          <?php
            while($rowAlldate = mysqli_fetch_assoc($resultAlldate)) { ?>
              <option value="<?php echo $rowAlldate['req_date']; ?>"> <?php echo $rowAlldate['req_date']; ?> </option>
          <?php
            }
          ?>
        </select>
...
<div class="btn_next_pre">
          ...
          <a class="btnpg" href="receiverList.php?x=<?php echo ($a-1); ?>" >Previous</a>
          <?php endif; ?>

          <?php
            for($i=1; $i<=$np; $i++){ ?>
              <a class="btnpg" href="receiverList.php?x=<?php echo $i; ?>"><?php echo $i; ?></a>
          <?php
            }
          ?>
        <?php if (($perPage+$startingRow) <= ($totalc-1)): ?>
          <a class="btnpg" href="receiverList.php?x=<?php echo ($a+1); ?>" >Next</a>
          <?php endif; ?>  
